...
function pictureRoute() {
  ...
  tweet.monitor(function(tweet) {
    var tag = /(#[0-9]{1,2})/.exec(tweet.text);
    if (tag) {
      var data = {
        tag: tag[1],
        meme: {
          top: tweet.text.replace(/#.+?\s/g, ''),
          bottom: '#Devox2014 by ' + tweet.user.screen_name
        }
      }
      handleMeme(data);
    }
  ...
  function handleMeme(data, callback) {
    ...
    $fh.db({
      ...
      if (err) {
        winston.warn('Could not find tag ' + err);
      } else {
        ...
        meme.create(file, data.meme, function(picture) {
