var baseUrl = "https://engdev-5uajb5mjscmsprhn4m6flrlv-evals-dev.mbaas1.tom.redhatmobile.com";
...
function pictureRoute() {
  ...
  picture.get('/tags', function(req, res) {
    winston.info('Listing memes for tag :', req.query.tag);
    $fh.db({
      "act": "list",
      "type": "meme",
      "eq": {
    ...
    $fh.db({
      ...
      if (err) {
        ...
        fs.writeFile(saveTo, data, 'base64', function (err) {
          ...
          var url = baseUrl + '/cloud/picture/org/' + entity.guid + '.jpg';
          tweet.update('Tweet a meme #Jazoon #RHTDX ' + tag, url);
          res.json({
            status: "ok",
  ...
  picture.post('/retweet', function (req, res) {
    ...
    $fh.db({
      ...
      if (err) {
        ...
        tweet.update(entity.meme.top + ' #Jazoon #RHTDX ' + entity.tag, url);
        res.json({
          status: "ok",
          message: {meme: entity}
        });
    ...
    $fh.db({
      ...
      if (err || found.list.length === 0) {
        ...
        meme.create(file, data.meme, function (err, picture) {
          ...
          var url = baseUrl + path.basename(picture);
